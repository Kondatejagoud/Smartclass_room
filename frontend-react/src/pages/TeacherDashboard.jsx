...
function TeacherDashboard() {
  ...
  const setupWebSocket = (classId) => {
    ...
    wsRef.current.onmessage = async (event) => {
      ...
      else if (data.type === 'students_data') setStudents(data.students);
      ...
      else if (data.type === 'strike_added') addAlert(`Strike on Student ${data.student_id} (${data.strikes}/5) — ${data.reason}`);
  ...
  const sendChat = () => {
    ...
    setChatMessages(prev => [...prev, { sender: 'You (Teacher)', text: chatMessage, time: new Date().toLocaleTimeString() }]);
    setChatMessage('');
  };

  const addStrike = () => {
    if (!selectedStudent) return alert('Select a student');
    const reason = prompt('Reason for strike:', 'Manual strike');
    if (!reason) return;
    wsRef.current?.send(JSON.stringify({ type: 'add_strike', student_id: selectedStudent, reason }));
  };

  const timeoutStudent = (minutes) => {
    if (!selectedStudent) return alert('Select a student');
    wsRef.current?.send(JSON.stringify({ type: 'timeout_student', student_id: selectedStudent, minutes }));
  };

  const removeStudent = () => {
    if (!selectedStudent) return alert('Select a student');
    const reason = prompt('Reason for removal:', 'Disruptive behavior');
    if (!reason) return;
    wsRef.current?.send(JSON.stringify({ type: 'remove_student', student_id: selectedStudent, reason }));
  };
  ...
  const broadcastVideoOffer = async () => {
    ...
    if (wsRef.current?.readyState === WebSocket.OPEN)
      wsRef.current.send(JSON.stringify({ type: 'webrtc_offer', offer }));
    ...
    <div className="sc-root">
      ...
      <nav className="sc-nav">
        ...
        <div className="sc-nav-right">
          <span className="sc-nav-user"><i className="fas fa-user-circle"></i> {localStorage.getItem('username')}</span>
          {!created
            ...
            <div className="sc-video-area">
              ...
              <div className="sc-video-grid">
                <div className="sc-tile sc-tile-teacher">
                  ...
                  <div className="sc-tile-bar">
                    <span className="sc-tile-name"><i className="fas fa-star"></i> {localStorage.getItem('username')} (You)</span>
                ...
                {students.length === 0 ? (
                  ...
                  students.map(student => (
                    <div key={student.user_id} className="sc-tile">
                      <div className="sc-tile-avatar sc-tile-avatar-student">
                        <i className="fas fa-user-graduate"></i>
                      </div>
                      <div className="sc-tile-bar">
                        <span className="sc-tile-name">Student #{student.user_id}</span>
              ...
              <div className="sc-controls">
                ...
                <div className="sc-controls-center">
                  ...
                  <button className="sc-ctrl-btn" title="Share Screen">
                ...
                <div className="sc-panel-body">
                  ...
                  <div className="sc-participant-item sc-participant-teacher">
                    ...
                    <div className="sc-participant-info">
                      <span className="sc-participant-name">{localStorage.getItem('username')}</span>
                  ...
                  {students.length > 0 && (
                    <div className="sc-discipline">
                      <p className="sc-discipline-title"><i className="fas fa-shield-alt"></i> Discipline — {selectedStudent ? `Student #${selectedStudent}` : 'Select a student above'}</p>
                      <div className="sc-discipline-grid">
                        <button className="sc-btn sc-btn-warn sc-btn-sm" onClick={addStrike}><i className="fas fa-exclamation-triangle"></i> Strike</button>
                        <button className="sc-btn sc-btn-ghost sc-btn-sm" onClick={() => timeoutStudent(5)}><i className="fas fa-clock"></i> 5 min</button>
                        <button className="sc-btn sc-btn-ghost sc-btn-sm" onClick={() => timeoutStudent(10)}><i className="fas fa-clock"></i> 10 min</button>
                        <button className="sc-btn sc-btn-danger sc-btn-sm" onClick={removeStudent}><i className="fas fa-user-times"></i> Remove</button>
                      </div>
                    </div>
                  )}
                </div>
